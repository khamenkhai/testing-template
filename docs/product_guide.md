...
│       ├── auth.seeder.ts          # Programmatic seeder
│       └── seeder.module.ts
│
...
| `ApiResponseInterceptor` | `APP_INTERCEPTOR` | Wraps all responses in `{ success, message, data }` |
...
| PATCH  | `/api/v1/admin/users/:id/role`                          | JWT  | —             | `UsersController.assignRole`         |
| POST   | `/api/v1/admin/roles`                                   | —    | —             | `RolesController.createRole`         |
| GET    | `/api/v1/admin/roles`                                   | —    | —             | `RolesController.findAllRoles`       |
| GET    | `/api/v1/admin/roles/:id`                               | —    | —             | `RolesController.findOneRole`        |
| PATCH  | `/api/v1/admin/roles/:id`                               | —    | —             | `RolesController.updateRole`         |
| DELETE | `/api/v1/admin/roles/:id`                               | —    | —             | `RolesController.deleteRole`         |
| GET    | `/api/v1/admin/roles/permissions/all`                   | —    | —             | `RolesController.findAllPermissions` |
| DELETE | `/api/v1/admin/roles/:roleId/permissions/:permissionId` | —    | —             | `RolesController.removePermission`   |
...
| `User`           | `users`           | `id` (UUID v7), `username`, `email` (unique), `roleId?` (FK→roles), `password?` (select: false), `refreshToken?`, `isDeleted`, timestamps |
| `Todo`           | `todos`           | `id` (UUID v7), `title`, `description?`, `isCompleted`, `isDeleted`, `image?`, `userId?` (FK→users), timestamps                           |
...
```typescript
// Find one
this.prisma.todo.findFirst({ where: { id, userId } });
...
Body: { email, username, password, roleId? }
```

1. Check if user already exists → throw `BadRequestException` if yes
2. Validate `roleId` if provided
...
7. Return `{ access_token, refresh_token, user: { id, email, role } }`
...
| Users  | `AssignRoleDto` | `roleId` (IsUUID)                                                                                         |
| Todo   | `CreateTodoDto` | `title` (IsString), `description?` (IsString), `isCompleted?` (IsBoolean, with Transform), `image?` (any) |
...
```typescript
// In service
async findAll(userId: string, page: number, limit: number): Promise<PaginatedResponse<Todo>> {
  const skip = (page - 1) * limit;
  const [items, totalItems] = await Promise.all([
    this.prisma.todo.findMany({ where: { userId }, take: limit, skip }),
    this.prisma.todo.count({ where: { userId } }),
  ]);
...
intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
  ...
  return next.handle().pipe(
    map((res) => {
      ...
      return { success: true, message: responseMessage, data: res };
    }),
  );
}
```

The interceptor wraps any response that already has a `data` property, preserving `meta` for paginated results. It falls back to a default message per HTTP method if no `@ResponseMessage()` decorator is set.
...
- **Permissions**: `todo.create`, `todo.read`, `todo.update`, `todo.delete`, `user.create`, `user.read`, `user.update`, `user.delete`
- **Role**: `superadmin` with all permissions linked
- **User**: `admin@example.com` / `Password123!` with superadmin role

### Programmatic Seeder

`src/database/seeders/auth.seeder.ts`

Injectable class with `seed()` method — can be used in tests or on-init hooks.
...
export class YourFeatureService {
  ...
  async create(dto: CreateYourFeatureDto) {
    return this.prisma.yourModel.create({ data: dto });
  }
  ...
  async update(id: string, dto: UpdateYourFeatureDto) {
    ...
    return this.prisma.yourModel.update({ where: { id }, data: dto });
  }
...
Then run `npm run prisma:migrate -- --name add_your_model`.
